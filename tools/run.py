...
def insert_accounts(conn,numamount):
    ...
    for i in range(1,numamount+1):
        if i%2 != 0:
            ...
            toId = i+1
        with conn.cursor() as cur:
            cur.execute("""UPSERT INTO accounts (id, balance,hashstr,province,address,ssn,company,date_time,dar,text_type,text_4k,text_42) VALUES 
            (%s,1000,%s,%s,%s,%s,%s,%s,ARRAY[%s,%s,%s],%s,%s,%s), 
            (%s, 250,%s,%s,%s,%s,%s,%s,ARRAY[%s,%s,%s],%s,%s,%s)
            """\
            ,(fromId,\
            ...
            fd.random_letter(),fd.text(),hashlib.sha512(str(fromId).encode("utf-8")).hexdigest().upper()*50,\
            hashlib.sha512(str(fromId).encode("utf-8")).hexdigest().upper()*50,\
            toId, \
            ...
            fd.text(),hashlib.sha512(str(fromId).encode("utf-8")).hexdigest().upper()*50,\
            hashlib.sha512(str(fromId).encode("utf-8")).hexdigest().upper()*50)\
            )
            if count%1000 == 0:
                print(count)
