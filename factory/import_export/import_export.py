...
class Base_bk(Startup):
    def __init__(self,bktype="import_export"):
        self.hainfo = bktype
        ...
        self.localpath = config.get(self.qianbase,"localpath").strip()
        self.httppath = config.get(self.qianbase,"httppath").strip()
        ...
        self.httpport = config.get(self.qianbase, "httpport").strip()
        self.httpuser = config.get(self.qianbase, "httpuser").strip()
        self.httppw = config.get(self.qianbase, "httppw").strip()
        self.httpexpath = config.get(self.qianbase, "httpexpath").strip()
        
        
    
    def returnpara(self):
        dic = {'dbname':self.dbname,'securepw':self.securepw,'user':self.user,'sshpassword':self.password,'host':self.host,'dbport':self.listenaddr,'sshport':self.sshport,'bindir':self._bindir,'httppath':self.httppath,'localpath':self.localpath,"securemode" : self.securemode,"gm":self.gm,"client":self.client_ip,'httpport':self.httpport,'httpuser':self.httpuser,'httppw':self.httppw,'httpexpath':self.httpexpath}
        return dic 
    ...
    #install import_export qianbasedb
