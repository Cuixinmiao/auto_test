...
class Startup():
    ...
    def __init__(self,fd={}):
        ...
        try:
            ...
            self.toolspathinst = os.path.join(self.toolspath,self.hainfo)
            ...
            self.libgeos = os.path.join(self.toolspathinst,"libgeos.so")
            self.libgeos_c = os.path.join(self.toolspathinst,"libgeos_c.so")
            ...
            if "-" not in self.advertiseaddr1[0]:
                ...
                for tmpidx in range(len(self.advertiseaddr1)):
                    ...
                    if tmpidx == len(self.advertiseaddr) - 1:
                        ...
                        for index in range(0,int(num)):
                            self.join=self.join + self.advertiseaddr[tmpidx] + ":" + str(int(self.listenaddr) + index) + sep
            self.execbin = os.path.join(self.toolspathinst,self._binname) 
            ...
            if fd.get("listenaddr") and fd.get("advertiseaddr"):
                ...
                self.join = jointmp
            if fd.get("binname"):
                self.execbin = os.path.join(self.toolspathinst,self.binname)
            if fd.get("user"):
    ...
    def run(self,hatype=None):
        ...
        for ip_num in self.startnum:
            ...
            for j in range(0,int(num)):
                ...
                cmd = """%s start --%s --store=%s --timezone=%s --lock-table-size=%s --advertise-addr=%s --listen-addr=:%s --http-addr=:%s --locality=%s --join=%s --cache=%s --max-sql-memory=%s --background"""\
                        %(self.binname,self.securemode,datadir,self.timezone,self.locksize,ip,str(int(self.listenaddr)+j),str(int(self.httpaddr)+j),locality,self.join1,self.cache,self.maxsqlmemory)
                #print(cmd)
                #else:
                #    pass
                try:
    ...
    def pre_env(self):
        ...
        self.upload_scp(self.libgeos,"/usr/local/lib/qianbase")
        self.upload_scp(self.libgeos_c,"/usr/local/lib/qianbase")
    ...
    def upload_scp(self,localpath,remotepath):
        ...
        try:
            if os.path.exists(localpath):
                for ip in listip:
                    sshapp.cmd("mkdir -p %s"%(remotepath),ip)
...
def upload_scp(localpath=None,remotepath="/etc/",host=None,hatype="ha331"):
    ...
    if not host:
        ...
        try:
            if os.path.exists(localpath):
                for ip in listip:
                    rmt_cmd = "mkdir -p %s/qianbase;scp -r -P %s %s %s@%s:%s/qianbase"%(remotepath,obj.sshport,localpath,obj.user,ip,remotepath)
