...
def case20_qianbase():
    try:
        ...
        #导出的数据没有后插入的两条
        expdata = execcmd.dealwithstr(ssh.myssh(execcmd.execcmd(exdatacomm,host,dbport),ssharg))
        expdata[:] = [expdata[0]] +  expdata[2:]
        ...
        importcmd = "use %s;IMPORT INTO %s (%s, %s) CSV DATA (%s) WITH skip = '1';" %(newdb,tabname,t2[2]['column_id'],t2[2]['column_balance'],importstr)
