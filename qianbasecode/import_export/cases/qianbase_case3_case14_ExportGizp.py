...
def case3_14_qianbase():
    try:
        ...
        #执行数据文件
        command,command2,d  = execcmd.execdatafile(bindir,host,dbport,sonpath)
        ...
        #导出        
        exportcmd = "EXPORT INTO CSV '%s' WITH compression = 'gzip' FROM TABLE %s.%s;" %(localpath,newdb,tabname)
        ...
        #执行数据文件
        t = execcmd.execdatafile(bindir,host,dbport,sonpath2)
        ssh.myssh(t[0],ssharg)
    
        importcmd = "use %s;IMPORT INTO %s (%s, %s) CSV DATA (%s) WITH decompress = 'gzip';" %(newdb,tabname,t[2]['column_id'],t[2]['column_balance'],importstr)
