...
def case2_qianbase():
    try:
        ...
        #导出        
        exportcmd = "EXPORT INTO CSV '%s'WITH chunk_rows = '1' from select * FROM  %s.%s;" %(localpath,newdb,tabname)
        ...
        count = 'cd %s;ls | wc -l' %(inbackpath)   
        count2 = ssh.myssh(count,newssharg).strip('\n')
        #导入
        #拷贝文件
        ...
        write_logger(logfile).info('\ncsv file nums:%s' %(count2))
        assert count2 == "2",'CSV data sum error'
