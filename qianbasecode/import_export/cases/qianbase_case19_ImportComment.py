...
def case19_qianbase():
    try:
        ...
        sonpath = os.path.abspath(os.path.join(basepath, "qianbasecode/import_export/PrepareTheData/DataTableWithComment.sql"))
        sonpath2 = os.path.abspath(os.path.join(basepath, "qianbasecode/import_export/PrepareTheData/StructTableWithComment.sql"))
        ...
        #导出        
        exportcmd = "EXPORT INTO CSV '%s'  FROM TABLE %s.%s;" %(localpath,newdb,tabname)
        ssh.myssh(execcmd.execcmd(exportcmd,host,dbport),ssharg)
        #再插入两条数据
        insecomm = "use %s;INSERT INTO %s VALUES ('4', 4),('5', 5);" % (newdb,tabname)
        ...
        #导出的数据没有后插入的两条
        expdata = execcmd.dealwithstr(ssh.myssh(execcmd.execcmd(exdatacomm,host,dbport),ssharg))[:-3]
        ...
        importcmd = "use %s;IMPORT INTO %s (%s, %s) CSV DATA (%s) WITH comment = '#';" %(newdb,tabname,t2[2]['column_id'],t2[2]['column_balance'],importstr)
