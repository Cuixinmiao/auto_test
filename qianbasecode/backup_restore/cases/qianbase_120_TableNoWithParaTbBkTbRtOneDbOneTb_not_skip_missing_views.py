...
# description : 表级备份恢复，单database单table表级备份恢复无with参数，恢复无with参数
#################################
...
def case120_qianbase():
    try:
        ...
        res = ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg,flag=1)
        print(res)

        #查恢复后情况
        assert b'without restoring referenced table (or "skip_missing_views" option)' in res,'no pass'
