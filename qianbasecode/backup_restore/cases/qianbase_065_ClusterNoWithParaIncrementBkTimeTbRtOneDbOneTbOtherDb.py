...
# description :  集群级备份表级时间点恢复，单database单table增量备份时间点恢复无with参数，时间点恢复表恢复到其它库下with into_db=xxx
#################################
...
def case65_qianbase():
    try:
        ...
        if gm == 'gm':
            ...
            with_para_rt = "with gm_encryption,into_db="
        else:
            with_para_bk = "with revision_history"
            with_para_rt = "with into_db="
        ...
        ###校验1### 
        db1 = 'n' * 1 + newdb
        createcom = 'use %s;create database %s' %(dbname,db1)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)
        #进行时间点恢复，恢复到time0
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s%s;" % (newdb,tabname,backuppath, backfile,time0,with_para_rt,db1)
        ...
        #查恢复后的数据、表结构
        excommand = 'use %s;show create table %s' % (db1,tabname)
        exdatacomm = 'use %s;select * from %s' % (db1,tabname)
        ...
        ###校验2###
        db2 = 'n' * 2 + newdb
        createcom = 'use %s;create database %s' %(dbname,db2)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)
        #进行时间点恢复，恢复到time1
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s%s;" % (newdb,tabname,backuppath, backfile,time1,with_para_rt,db2)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)
        #查恢复后的数据、表结构
        excommand = 'use %s;show create table %s' % (db2,tabname)
        exdatacomm = 'use %s;select * from %s' % (db2,tabname)
        des = execsql.dealwithstr(ssh.myssh(execsql.execcmd(excommand,backip,backdbport),newssharg))
        desdata = execsql.dealwithstr(ssh.myssh(execsql.execcmd(exdatacomm,backip,backdbport),newssharg))
        ...
        ###校验3###
        db3 = 'n' * 3 + newdb
        createcom = 'use %s;create database %s' %(dbname,db3)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)
        #进行时间点恢复，恢复到time2
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s%s;" % (newdb,tabname,backuppath, backfile,time2,with_para_rt,db3)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)

        #查恢复后的数据、表结构
        excommand = 'use %s;show create table %s' % (db3,tabname)
        exdatacomm = 'use %s;select * from %s' % (db3,tabname)
        des = execsql.dealwithstr(ssh.myssh(execsql.execcmd(excommand,backip,backdbport),newssharg))
        desdata = execsql.dealwithstr(ssh.myssh(execsql.execcmd(exdatacomm,backip,backdbport),newssharg))
        #对比表结构、数据
        write_logger(logfile).info('\nexp:%s \ndes:%s' %(exp,des))
        ...
        ###校验4###
        db4 = 'n' * 4 + newdb
        createcom = 'use %s;create database %s' %(dbname,db4)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)
        #进行时间点恢复，恢复到time0
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s%s;" % (newdb,tabname,backuppath, backfile,time3,with_para_rt,db4)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)

        #查恢复后的数据、表结构
        excommand = 'use %s;show create table %s' % (db4,tabname)
        exdatacomm = 'use %s;select * from %s' % (db4,tabname)
        des = execsql.dealwithstr(ssh.myssh(execsql.execcmd(excommand,backip,backdbport),newssharg))
        desdata = execsql.dealwithstr(ssh.myssh(execsql.execcmd(exdatacomm,backip,backdbport),newssharg))
        #对比表结构、数据
        write_logger(logfile).info('\nexp:%s \ndes:%s' %(exp,des))
