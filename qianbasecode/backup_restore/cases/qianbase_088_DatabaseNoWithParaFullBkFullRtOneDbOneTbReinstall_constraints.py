...
# description : 全备份全恢复，单database单table全备份全恢复无with参数，拷贝重装恢复无with参数,涉及中英文参数和约束
#################################
...
def case88_qianbase():
    try:
        ...
        excommentcom = 'use %s;SHOW DATABASES WITH COMMENT;' %(dbname)
        exp = execsql.dealwithstr(ssh.myssh(execsql.execcmd(excommand,host,dbport),ssharg))
        expcomment = execsql.dealwithstr(ssh.myssh(execsql.execcmd(excommentcom,host,dbport),ssharg))
        #备份的数据
        ...
        descomment = execsql.dealwithstr(ssh.myssh(execsql.execcmd(excommentcom,host,dbport),ssharg))
        ...
        write_logger(logfile).info('\nexpcomment:%s \ndescomment:%s' %(expcomment,descomment))
        assert expcomment == descomment,'库级comment不正确'
