...
# description : 集群备份集群恢复，单database单table全量备份with encryption_passphrase全量恢复无with参数，拷贝重装恢复无with参数 
#################################
...
def case74_qianbase():
    try:
        ...
        if gm == 'gm':
            with_para_bk = "with gm_encryption,encryption_passphrase='test@123!'"
            with_para_rt = "with gm_encryption,encryption_passphrase='test123!'"
        else:
            with_para_bk = "with encryption_passphrase='test@123!'"
            with_para_rt = "with encryption_passphrase='test123!'"
        ...
        print(ret)
        assert ret == b'ERROR: failed to decrypt \xe2\x80\x94 maybe incorrect key: cipher: message authentication failed\nFailed running "sql"\n',nopass
