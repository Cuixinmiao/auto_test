...
# author      : caihailong
# copyright   : esgyn
# date file   : 当前路径下的/PrepareTheData/datatypeall.sql
# description : 集群级备份恢复，单database单table全量备份恢复无with参数，拷贝重装恢复无with参数,所有数据类型
#################################
...
try:
    ...
    from factory.conf import *
    from factory.backup_restore import backup_restore
def case114_qianbase():
    try:
        basepath = os.getcwd()
        prepath = os.path.abspath(os.path.join(basepath, "qianbasecode/backup_restore/PrepareTheData/"))
        
        #获取OneDatabaseOneTable.sql的数据文件
        sonpath = os.path.abspath(os.path.join(basepath, "qianbasecode/backup_restore/PrepareTheData/datatypeall.sql"))
