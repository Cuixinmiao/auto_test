...
# description : 集群级备份表级时间点恢复，单database单table增量备份时间点改名恢复无with参数，时间点恢复无with参数
#################################
...
def case66_qianbase():
    try:
        ...
        #进行时间点恢复，恢复到time0
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s;" % (newdb,tabname,backuppath, backfile,time0,with_para_rt)
        ...
        ###校验2###
        renamecom = 'use %s;alter table %s rename to newtable2' %(newdb,tabname)
        ssh.myssh(execsql.execcmd(renamecom,host,dbport),ssharg)
        #进行时间点恢复，恢复到time1
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s;" % (newdb,tabname,backuppath, backfile,time1,with_para_rt)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)
        #查恢复后的数据、表结构
        ...
        ###校验3###
        #改表名
        renamecom = 'use %s;alter table %s rename to newtable3' %(newdb,tabname)
        ssh.myssh(execsql.execcmd(renamecom,host,dbport),ssharg)
        #进行时间点恢复，恢复到time2
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s;" % (newdb,tabname,backuppath, backfile,time2,with_para_rt)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)
        ...
        ###校验4###
        renamecom = 'use %s;alter table %s rename to newtable4' %(newdb,tabname)
        ssh.myssh(execsql.execcmd(renamecom,host,dbport),ssharg)
        #进行时间点恢复，恢复到time0
        rescommand = "RESTORE table %s.%s FROM '%s%s' AS OF SYSTEM TIME '%s' %s;" % (newdb,tabname,backuppath, backfile,time3,with_para_rt)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)
