...
# author      : caihailong
# copyright   : esgyn
# date file   : 当前路径下的/PrepareTheData/ch_en.sql
# description : 库级备份库级恢复，单database单table库备份表级恢复无with参数，改原库名恢复无with参数,涉及中英文参数
#################################
...
def case84_qianbase():
    try:
        ...
        sonpath = os.path.abspath(os.path.join(basepath, "qianbasecode/backup_restore/PrepareTheData/ch_en.sql"))
        ...
        #进行备份
        bakcommand = "BACKUP database %s INTO '%s' in 'nodelocal://%s/%s'  %s;" % (newdb,path,nodeid, backdir,with_para_bk)
        ssh.myssh(execsql.execcmd(bakcommand,host,dbport),ssharg)
        #等待备份完成
        execsql.judgecmd("BACKUP",ssharg,3)

        #再插入一条数据
        insecomm = '''use %s;INSERT INTO %s values(51,'关浩','湖南省桂芝县上街周街m座 663767','其他','2002-06-24','192.168.133.83','bduan@gmail.com','EZJSzjFy86');''' % (newdb,tabname)
        ...
        #备份的数据没有后插入的两条
        expdata = execsql.dealwithstr(ssh.myssh(execsql.execcmd(exdatacomm,host,dbport),ssharg))[:-1]
        ...
        #改名
        createcom= 'use %s;alter  DATABASE  %s rename to  new%s ;' %(dbname,newdb,newdb)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)
       
         #进行恢复
        backuppath = eval(backuppath)
        rescommand = "RESTORE database %s FROM '%s/%s' %s;" % (newdb,backuppath, path,with_para_rt)
