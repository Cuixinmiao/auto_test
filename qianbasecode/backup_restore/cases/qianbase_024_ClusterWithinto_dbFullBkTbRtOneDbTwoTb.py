...
# description : 集群级备份表级恢复，单database两table全量备份无with参数表级恢复，表恢复(其中一张)到其它库下with into_db=xxx
#################################
...
def case24_qianbase():
    try:
        ...
        #创建新库
        newdb2 = 'new' + newdb
        createcom = 'use %s;create database %s;' %(dbname,newdb2)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)

        #进行恢复
        backuppath = eval(backuppath)
        rescommand = "RESTORE table %s.%s FROM '%s%s' %s%s;" % (newdb,tabname,backuppath, backfile,with_para_rt,newdb2)
        ...
        #查恢复后的数据、表结构
        excommand = 'use %s;show create table %s' % (newdb2,tabname)
        exdatacomm = 'use %s;select * from %s' % (newdb2,tabname)
