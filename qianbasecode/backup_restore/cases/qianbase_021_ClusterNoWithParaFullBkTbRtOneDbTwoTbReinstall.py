...
# description :  集群级备份表级恢复，单database两table全量备份表级恢复无with参数，拷贝重装新建同名库恢复无with参数
#################################
...
def case21_qianbase():
    try:
        ...
        rescommand = "RESTORE table %s.%s, %s.%s FROM '%s%s'  %s;" % (newdb,tabname,newdb,tabname2,backuppath, backfile,with_para_rt)
