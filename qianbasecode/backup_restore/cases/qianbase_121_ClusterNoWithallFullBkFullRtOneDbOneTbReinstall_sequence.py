...
def case90_qianbase():
    try:
        ...
        if gm == 'gm':
            ...
            with_para_rt = "with gm_encryption,skip_missing_sequences"
        else:
            with_para_bk = ""
            with_para_rt = "with skip_missing_sequences"
        ...
        rename = 'use %s;alter table %s rename to new%s' %(newdb,tabname,tabname)
        ...
        assert 'nextval' in str(exp) and 'nextval' not in str(des),'表结构不匹配'
