...
# author      : caihailong
# copyright   : esgyn
# date file   : 当前路径下的/PrepareTheData/view.sql
# description : 集群备份集群恢复，单database单table单sequence全量备份无with参数，拷贝重装恢复无with参数 
#################################
...
def case90_qianbase():
    try:
        ...
        sonpath = os.path.abspath(os.path.join(basepath, "qianbasecode/backup_restore/PrepareTheData/sequence.sql"))
        ...
        seqcmd1 = "use %s;select nextval('%s');select currval('%s');" %(newdb,seqname,seqname)
        expseqcurrval = ssh.myssh(execsql.execcmd(seqcmd1,host,dbport),ssharg)
        
        seqcmd2 = "use %s;SELECT * FROM %s;" %(newdb,seqname)
        expseq = ssh.myssh(execsql.execcmd(seqcmd2,host,dbport),ssharg)


        #再插入一条数据
        insecomm = "use %s;insert into %s (id,name) values(5,'e');" % (newdb,tabname)
        ...
        #备份的数据没有后插入的两条
        expdata = execsql.tabledata(ssharg,newdb,tabname)[:-1]
        ...
        desseqcurrval = ssh.myssh(execsql.execcmd(seqcmd1,host,dbport),ssharg)
        desseq = ssh.myssh(execsql.execcmd(seqcmd2,host,dbport),ssharg)
        ...
        write_logger(logfile).info('\nexpseqcurrval:%s \ndesseqcurrval:%s' %(expseqcurrval,desseqcurrval))
        assert expseqcurrval == desseqcurrval,'表结构不匹配'
        write_logger(logfile).info('\nexpseq:%s \ndesseq:%s' %(expseq,desseq))
        assert expseq == desseq,'数据不匹配'
