...
# description : 集群备份集群恢复，单database单table全量备份with DETACHED全量恢复无with参数，拷贝重装恢复无with参数 
#################################
...
def case73_qianbase():
    try:
        ...
        if gm == 'gm':
            with_para_bk = "with gm_encryption,DETACHED,encryption_passphrase = 'password123',revision_history"
            with_para_rt = "with gm_encryption,DETACHED,encryption_passphrase = 'password123'"
        else:
            with_para_bk = "with DETACHED,encryption_passphrase = 'password123',revision_history"
            with_para_rt = "with DETACHED,encryption_passphrase = 'password123'"
