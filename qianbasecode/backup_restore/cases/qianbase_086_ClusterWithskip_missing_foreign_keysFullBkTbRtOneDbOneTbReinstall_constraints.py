...
# description : 集群级备份表级恢复，单database单table全量备份库表恢复with skip_missing_foreign_keys参数，删除其中一张表恢复with skip_missing_foreign_keys参数,涉及中英文参数和约束
#################################
...
def case86_qianbase():
    try:
        ...
        if gm == 'gm':
            ...
            with_para_rt = "with gm_encryption,skip_missing_foreign_keys"
        else:
            with_para_bk = ""
            with_para_rt = "with skip_missing_foreign_keys"
        ...
        assert 'FOREIGN KEY' in str(exp)  and 'FOREIGN KEY' not in str(des),'表结构不正确'
