...
def case1_qianbase():
    try:
        ...
        grantcmd = 'CREATE USER esgyn WITH PASSWORD qianbase;GRANT ALL ON DATABASE %s TO esgyn;GRANT DELETE ON TABLE %s.%s TO esgyn;' %(newdb,newdb,tabname)
        ssh.myssh(execsql.execcmd(grantcmd,host,dbport),ssharg)

        expgrant = ssh.myssh(execsql.execcmd('SHOW GRANTS ON DATABASE %s;SHOW GRANTS ON TABLE %s.%s;' %(newdb,newdb,tabname),host,dbport),ssharg)
        #进行备份
        ...
        desgrant = ssh.myssh(execsql.execcmd('SHOW GRANTS ON DATABASE %s;SHOW GRANTS ON TABLE %s.%s;' %(newdb,newdb,tabname),host,dbport),ssharg)
