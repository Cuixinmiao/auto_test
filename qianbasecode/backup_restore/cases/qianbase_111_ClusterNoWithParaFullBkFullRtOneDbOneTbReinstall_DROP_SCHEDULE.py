...
def case1_qianbase():
    try:
        ...
        ssh.myssh(execsql.execcmd(execsql.schoper(schename,'DROP'),host,dbport),ssharg)
        c = execsql.judgesch(schename, ssharg, incretime,'DROP')
        print(c)
        write_logger(logfile).info('\nexp DROP nums:%s \ndes DROP nums:%s' %(c,'[]'))
        assert c == [],'schedule did not drop success'
