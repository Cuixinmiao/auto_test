...
def case100_qianbase():
    try:
        ...
        createcmd = 'create table %s (w_id integer not null,w_ytd  decimal(12,2),w_tax decimal(4,4),w_name varchar(10),w_street_1  varchar(20),w_street_2  varchar(20),w_city varchar(20),w_state char(2),w_zip char(9),index %s(w_id));' %(tabname,idxname)
        print(createcmd) 
        s = 'alter index %s@%s partition by range(w_id)(' %(tabname,idxname)
        s1 = ''
        c = 0
        for i in rackall:
            s += 'PARTITION p0_%s VALUES FROM (%d) TO (%d),' %(str(c),c,c+125)
            s1 += "ALTER PARTITION p0_%s OF index %s@%s CONFIGURE ZONE USING constraints = '[+%s]';" %(str(c),tabname,idxname,i)
            c += 125
        s = s.strip(',') + ');'
        print(s)
        print(s1)
