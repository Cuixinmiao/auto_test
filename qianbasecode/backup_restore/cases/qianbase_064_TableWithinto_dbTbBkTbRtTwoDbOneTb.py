...
# description : 表级备份表级恢复，两database单table表级备份无with参数表级恢复，表恢复到其它库下with into_db=xxx
#################################
...
def case64_qianbase():
    try:
        ...
        #进行备份
        bakcommand = "BACKUP table %s.%s,%s.%s INTO 'nodelocal://%s/%s'  %s;" % (newdb,tabname,newdb2,tabname2,nodeid, backdir,with_para_bk)
        ...
        #创建新库
        newdbplus =  newdb + newdb2
        createcom = 'use %s;create database %s;' %(dbname,newdbplus)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)

        #进行恢复
        backuppath = eval(backuppath)
        rescommand = "RESTORE table %s.%s,%s.%s FROM '%s%s' %s%s;" % (newdb,tabname,newdb2,tabname2,backuppath, backfile,with_para_rt,newdbplus)
        ...
        #查恢复后的数据、表结构
        excommand = 'use %s;show create table %s' % (newdbplus,tabname)
        exdatacomm = 'use %s;select * from %s' % (newdbplus,tabname)
        ...
        excommand2 = 'use %s;show create table %s' % (newdbplus,tabname2)
        exdatacomm2 = 'use %s;select * from %s' % (newdbplus,tabname2)
