...
# description : 库级备份库级恢复，单database单table库级备份库级恢复无with参数，拷贝重装恢复无with参数,涉及中英文参数和约束
#################################
...
def case82_qianbase():
    try:
        ...
        renamecmd = 'use %s;alter database %s rename to new%s;' %(dbname,newdb,newdb)
        ssh.myssh(execsql.execcmd(renamecmd),ssharg)
        #进行恢复
