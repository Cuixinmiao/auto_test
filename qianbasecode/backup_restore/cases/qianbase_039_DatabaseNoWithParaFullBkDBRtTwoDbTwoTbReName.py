...
# description : 库级备份库级恢复，两database两table库级备份库级恢复无with参数，原库改名后直接恢复无with参数
#################################
...
def case39_qianbase():
    try:
        ...
        #修改原库名
        renamecom = 'use %s;ALTER DATABASE %s RENAME TO rebakndb_cur;' %(dbname,newdb)
        ssh.myssh(execsql.execcmd(renamecom,host,dbport),ssharg)
        #修改原库名
        renamecom2 = 'use %s;ALTER DATABASE %s RENAME TO rebakndb_cur2;' %(dbname,newdb2)
        ssh.myssh(execsql.execcmd(renamecom2,host,dbport),ssharg)
