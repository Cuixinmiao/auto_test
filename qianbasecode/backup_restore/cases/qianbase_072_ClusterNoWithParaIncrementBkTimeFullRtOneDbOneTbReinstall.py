...
def case72_qianbase():
    try:
        ...
        #进行时间点恢复，恢复到time1
        rescommand = "RESTORE FROM '%s%s' AS OF SYSTEM TIME '%s' %s;" % (backuppath, backfile,time1,with_para_rt)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)
        #查恢复后的数据、表结构
        ...
        #进行时间点恢复，恢复到time2
        rescommand = "RESTORE FROM '%s%s' AS OF SYSTEM TIME '%s' %s;" % (backuppath, backfile,time2,with_para_rt)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)
        ...
        #进行时间点恢复，恢复到time0
        rescommand = "RESTORE FROM '%s%s' AS OF SYSTEM TIME '%s' %s;" % (backuppath, backfile,time3,with_para_rt)
        ssh.myssh(execsql.execcmd(rescommand,host,dbport),ssharg)
        #等待恢复完成
        execsql.judgecmd("RESTORE", ssharg, 3)
