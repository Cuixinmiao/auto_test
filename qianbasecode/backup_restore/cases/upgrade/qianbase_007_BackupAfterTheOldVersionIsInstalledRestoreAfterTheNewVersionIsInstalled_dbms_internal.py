...
# description : 集群级备份恢复，单database单table全量备份恢复无with参数，拷贝重装恢复无with参数(先安装老版本，进行备份，然后卸载老版本重装新版本，进行恢复),生成备份前后的dbms_internal.transaction_statistics\dbms_internal.statement_statistics 进行前后的比较
#################################
...
def case6_qianbase():
    try:
        ...
        #备份的数据没有后插入的两条
        expdata = execsql.tabledata(ssharg,newdb,tabname)[:-2]        


        execsql.systbsituation(ssharg,'/tmp/bk_case6.txt')
        ...
        exptran = execsql.tablestructure(ssharg,newdb,'dbms_internal.transaction_statistics')
        expstat = execsql.tablestructure(ssharg,newdb,'dbms_internal.statement_statistics')
        ...
        execsql.systbsituation(ssharg,'/tmp/rs_case6.txt')
        
        #查恢复后的数据、表结构
        des = execsql.tablestructure(ssharg,newdb,tabname) 
        desdata = execsql.tabledata(ssharg,newdb,tabname) 
       
        destran = execsql.tablestructure(ssharg,newdb,'dbms_internal.transaction_statistics')
        desstat = execsql.tablestructure(ssharg,newdb,'dbms_internal.statement_statistics')
        ...
        write_logger(logfile).info('\nexptran:%s \ndestran:%s' %(exptran,destran))
        assert exptran == destran,'表结构不匹配'
        write_logger(logfile).info('\nexpstat:%s \ndesdatastat:%s' %(expstat,desstat))
        assert expstat == desstat,'数据不匹配'
        #检查状态是否正常
        execsql.checkstat(ssharg)
