...
# description : 集群级备份库级恢复，单database单table全量备份库级恢复无with参数，恢复无with参数(安装老版本，然后杀进程，替换新版本的二进制文件，重新拉起，进行备份和恢复)
#################################
...
def case5_qianbase():
    try:
        ...
        #改库名
        renamecom = 'use %s;alter database %s rename to new%s;' %(dbname,newdb,newdb)
