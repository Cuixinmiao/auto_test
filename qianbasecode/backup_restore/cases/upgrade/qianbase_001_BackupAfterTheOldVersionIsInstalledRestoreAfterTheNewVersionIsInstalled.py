...
# description : 集群级备份恢复，单database单table全量备份恢复无with参数，拷贝重装恢复无with参数(先安装老版本，进行备份，然后卸载老版本重装新版本，进行恢复)
#################################
...
def case1_qianbase():
    try:
        ...
        #检查ckpt是否正常
        ret = execsql.detectckpt(ssharg)
        print(ret)
        assert b'license for cluster(s)' in ret,'no pass'        
