...
# description : 表级备份表级恢复，单database单table全量备份表级恢复无with参数，改名恢复无with参数,涉及中英文参数
#################################
...
def case85_qianbase():
    try:
        ...
        #进行备份
        bakcommand = "BACKUP INTO '%s' in 'nodelocal://%s/%s'  %s;" % (path,nodeid, backdir,with_para_bk)
        ...
        #创建同名库
        createcom= 'use %s;alter table %s rename to new%s ;' %(newdb,tabname,tabname)
        ssh.myssh(execsql.execcmd(createcom,host,dbport),ssharg)
       
         #进行恢复
        backuppath = eval(backuppath)
        rescommand = "RESTORE table %s.%s FROM '%s/%s' %s;" % (newdb,tabname,backuppath, path,with_para_rt)
