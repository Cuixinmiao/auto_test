...
class Qianbasetestha331():
    ...
    def blade_patition_runtime_localback_qianbase(self):
        ...
        res = common.monitor_tpcc_runtime_netpatition(p,rid1,rid2,fn=fn,flag="blade")
        if res == True:
            assert res == True
        else:
            assert res[0] == 0,"delay greater than 26s errmsg:%s"%res[1]

    def blade_patition_localback_qianbase(self):
        try:
            rid1=None
            ipuuid=None
            # install qianbase ,set replicas 7 and register license
            ...
            for key2 in rid2info:
                rid2tmp.append(key2.split(":")[0])
            rid2 = list(set(rid2tmp))    
            ipuuid = common.setting_loss(rid1,rid2)
            time.sleep(60)
            #exec tpcc work business
            p = obj331.exec_tpcc_business()
