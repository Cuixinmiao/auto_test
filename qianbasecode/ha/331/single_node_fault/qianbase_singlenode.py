...
class Qianbasetestfunc():
    ...
    def single_node_fault_remote_qianbase(self):
    ...
    def single_node_fault_localback_qianbase(self):
        # install qianbase ,set replicas 7 and register license
        startup_ha.instha()
    ...
    def single_node_fault_primary_qianbase(self):
        # install qianbase ,set replicas 7 and register license
        startup_ha.instha()
    ...
    def single_node_fault_runtime_primary_qianbase(self):
        # install qianbase ,set replicas 7 and register license
        startup_ha.instha()
        ...
        m_name = sys._getframe().f_code.co_name
        fn=os.path.join(os.path.split(__file__)[0],m_name)
        res = common.monitor_tpcc_runtime(p,fn=fn,ipport=rid1single)
        if res == True:
            assert res == True
        else:
            assert res[0] == 0,"delay greater than 26s errmsg:%s"%res[1]
    ...
    def single_node_fault_runtime_localback_qianbase(self):
        # install qianbase ,set replicas 7 and register license
        startup_ha.instha()
        ...
        m_name = sys._getframe().f_code.co_name
        fn=os.path.join(os.path.split(__file__)[0],m_name)
        res = common.monitor_tpcc_runtime(p,fn=fn,ipport=rid2single)
        if res == True:
            assert res == True
        else:
            assert res[0] == 0,"delay greater than 26s errmsg:%s"%res[1]

    def single_node_fault_runtime_remote_qianbase(self):
        ## install qianbase ,set replicas 7 and register license
        startup_ha.instha()
        ...
        m_name = sys._getframe().f_code.co_name
        fn=os.path.join(os.path.split(__file__)[0],m_name)
        res = common.monitor_tpcc_runtime(p,fn=fn,ipport=rid3single)
        if res == True:
            assert res == True
        else:
            assert res[0] == 0,"delay greater than 26s errmsg:%s"%res[1]
