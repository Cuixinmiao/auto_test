...
def room_switch_init():
        ...
        startup_ha.wakeraft_primary_room(index=1)
        ##缺少检测主机房raft wake的机制，先暂定停30s
        #time.sleep(30)
        startup_ha.wakeraft_back_room(index=2)
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        #startup_ha.get_all_primary()
        #startup_ha.instha()
        #time.sleep(20)
        #exec_tpcc(init_ip,init_port,1000,3000)
        #time.sleep(20)
        #startup_ha.get_all_primary()
        room_switch_init()
        #startup_ha.get_all_primary()
        #time.sleep(5)
        #set_primary_room(rid=2)
        #time.sleep(5)
        #startup_ha.get_all_primary()
    if len(sys.argv) == 2:
        if sys.argv[1] == 'kill':
