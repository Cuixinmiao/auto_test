...
class Qianbasetestfunc():
    def single_node_fault_remote_qianbase(self):
        ...
        ipport = rid3single
        common.decommission_method(ipport) 
        
        rid2info = common.test_args()[1]
        for rid2single in list(rid2info.keys()):
            common.decommission_method(rid2single)
...
if __name__ == '__main__':
    ...
    try:
        ...
        #停止remote service
        #print("remote:%s"%ipport)
        common.decommission_method(ipport) 
        
        rid2info = common.test_args()[1]
        for rid2single in list(rid2info.keys()):
            print("remote:%s"%rid2single)
            common.decommission_method(rid2single)
